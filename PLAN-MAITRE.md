...
| G3 |​  ẟ3. Spec produit|PRD ：promesse, parcours utilisateur, features, monétisation|Le PRD|
...
| — |​ 5. Développement|`/speckit.implement` tâche par tâche ：1 tâche =1 session =1 PR revue CodeRabbit|—|
...
- **Bootstrap 3 fichiers** ：au démarrage de session, lire UNIQUEMENT AGENTS.md + SOMMAIRE.md
   ...
   brain/ en entier. Logs/rapports datés jamais re-lus en entier（on consulte le plus récent）。
- **Vocabulaire canonique** ：COMMANDES.md（RADAR, SCAN, DÉCORTIQUE, FORGE, STYLE, BÂTIT,
   GAUNTLET, EMBALLAGE, LANCE, PROMOUVOIS, PILOTE）— langage commun utilisateur/orchestrateur。
