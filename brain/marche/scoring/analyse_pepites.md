...
- **Verdict de faisabilité** pour notre factory React Native (Expo)
...
**Stack** :
- Option jeu : moteur 2D + assets déco, mécanique match/collection (React Native/Expo suffit, simple)
- Option outil 3D : **moteur 3D temps réel** (React Native + Three.js / expo-gl, ou Unity),
  gestion meubles 2D/3D, catalogues, AR (LiDAR) pour projection meubles
**Complexité** : 4/5 (la vraie 3D est complexe ; le jeu de déco est plus simple)
**Verdict** : le **jeu de déco** (Design Home-like) est faisable en React Native (Expo) ; l'outil 3D est plus
...
**Stack** : React Native (Expo) pur, local storage (MMKV/AsyncStorage) + éventuellement sync cloud. Listes, calendrier, streaks,
...
**Stack** : React Native (Expo) + local storage + notifications + widgets iOS. Simple.
**Complexité** : 1/5
...
**Stack** : React Native (Expo) + vidéos (hébergées ou embarquées) + suivi de progression + éventuellement
...
**Stack** : React Native (Expo) + vidéos/animations 2D de poses + timer + programmes.
**Complexité** : 2/5
...
**Stack** : React Native (Expo) pur + contenu (citations stoïques) + journal local + prompts quotidiens +
...
**Stack** : React Native (Expo) pur + contenu + journal + prompts + éventuellement IA (recommandations).
**Complexité** : 1/5
...
**Stack** : React Native (Expo) + **traitement audio (micro)** + analyse du son (détection ronflement) +
...
**Stack** : React Native (Expo) + caméra + traitement image (recadrage, suppression fond IA) + règles par pays.
...
**Stack** : React Native (Expo) + **traitement audio temps réel** (pitch, modulation) + natif audio.
...
React Native (Expo) (simple, monétisable, demande réelle, concurrence battable).
