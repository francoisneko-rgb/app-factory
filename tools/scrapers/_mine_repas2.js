...
let perApp={};
if(fs.existsSync(file)){perApp=JSON.parse(fs.readFileSync(file,'utf8'));}
const todo=[['LoseIt',5569619],['Cronometer',234680637185],['MyFitnessPal',5571082]];
(async()=>{
  for(const [name,pid] of todo){
    if(perApp[name] && !Array.isArray(perApp[name])===false) {}
    ...
    fs.writeFileSync(file,JSON.stringify(perApp,null,2),'utf8'); // write after each
    console.log('written');
  }
