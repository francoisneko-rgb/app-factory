// Avis App Store via flux RSS Apple (fiable, sans auth)
// Usage : node ios-reviews-rss.js <appId> <nombre> [pays=us]
// Récupère sortBy=mostHelpful puis sortBy=mostRecent (pages de 50, max 10 pages/tri).
const [,, id, numArg = '200', country = 'us'] = process.argv;
const num = Number(numArg);
if (!id) { console.error('Usage: node ios-reviews-rss.js <appId> <num> [country]'); process.exit(1); }
  ...
  for (const sort of ['mostHelpful', 'mostRecent']) {
    for (let page = 1; page <= 10; page++) {
      ...
      try {
        ...
        console.error(`Erreur page ${page} (${sort}) : ${e.message}`, process.stderr.write ? '' : '');
        break;
      }
