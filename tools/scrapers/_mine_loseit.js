...
(async()=>{
  const loseIt = await mineApp('LoseIt',5569619);
  console.log('LoseIt', loseIt.length);
  // relire le fichier existant ou construire depuis les valeurs déjà connues
  const brutDir='C:/MY_WORK/___Nouveau_BUSINESSES___/OpenCode/app-factory/brain/marche/mots-cles/_brut';
  const perApp = {};
  // re-miner rapidement tous pour être complet (rapide sauf LoseIt déjà fait)
  for(const [name,pid] of [['MealPrepPro',280347196252],['Mealime',42141783485],['eMeals',7854340133],['MyFitnessPal',5571082],['EatThisMuch',41352905878],['Cronometer',234680637185]]){
    perApp[name]=await mineApp(name,pid);
    console.log(name,perApp[name].length);
    await new Promise(r=>setTimeout(r,40));
  }
  perApp['LoseIt']=loseIt;
  fs.writeFileSync(brutDir+'/repas-nutrition.json',JSON.stringify(perApp,null,2),'utf8');
  console.log('SAVED repas-nutrition');
})();
